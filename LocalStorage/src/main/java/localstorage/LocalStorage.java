...
import storageSpecs.Configuration;
...
public class LocalStorage extends StorageSpecs {
    ...
    public boolean checkConfig(String root) {
        ...
        setConfig(new Configuration(getMaxSizeLimit(), getForbiddenExtensions(), getFileCountLimits()));
        return true;
    }
    ...
    public boolean addFile(String file) throws FileAlreadyExistsException, FileCountLimitException, MaxSizeLimitException {
        ...
        checkForbiddenExtension(original.getName());
    ...
    public List<MyFile> returnFileList(List<String> list) {
        ...
        for (String path : list) {
            ...
            MyFile printableFile = new MyFile(path, name, extension, createdAt, lastModifiedAt);
    ...
    @Override
    protected void checkFileCountLimit(String file) throws FileCountLimitException {
        ...
        if ((Long) getFileCountLimits().get(root.toPath().toString()) == Objects.requireNonNull(root.listFiles()).length) {
            throw new FileCountLimitException();
        }
    }

    @Override
    protected void checkMultipleFileCountLimit(String file, int count) throws FileCountLimitMultipleException {
        File root = new File(file);
        ...
        if ((Long) getFileCountLimits().get(root.toPath().toString()) < Objects.requireNonNull(root.listFiles()).length + count) {
            throw new FileCountLimitMultipleException();
        }
    }
